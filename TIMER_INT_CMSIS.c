...
/* Program to alternatively turn two LEDs ON and OFF when a switch is ON using Timer ISR */
...
{ /* When TIM2_ARR overflows, TIM2_SR: Bit 0 - UIF is Set (HIGH/1), this ISR is called.
	 Clear IT Bit and Toggle LEDs at GPIOB 6 & 7 (GPIOB_ODR: Bit 6 - ODR6 & Bit 7 - ODR7) */

	TIM_ClearITPendingBit(TIM2,TIM_IT_Update); // Clear TIM2 Interrupt
...
int main(void)
{
	...
	NVIC_EnableIRQ(TIM2_IRQn); // Enabling ARM Cortex M3 NVIC for Timer 2
